...
from graph_construction import GraphMethod, QuestionConditionedGraphBuilder, apply_question_conditioned_graph_batched
"""
...
class GroundTruth_SceneGraph_Encoder(torch.nn.Module):
    def __init__(self, graph_method="static", d_q=512, d_hidden=256, top_k=5):
        ...
        learned_methods = {m.value for m in GraphMethod} - {"static", "fully_connected"}
        if graph_method in learned_methods:
            self.graph_builder = QuestionConditionedGraphBuilder(
                method=graph_method,
                d_h=self.sg_emb_dim,
                d_e=self.sg_emb_dim,
                d_q=d_q,
    ...
    def forward(self,
        ...
        ##################################
        # Stage 2: Question-conditioned graph construction
        # Applied AFTER embedding, BEFORE the GNN encoding layer.
        # For 'static' / 'fully_connected', this is a no-op.
        ##################################
        if self.graph_builder is not None and q is not None:
            gt_scene_graphs, x_embed_sum, edge_attr_embed_sum = \
                apply_question_conditioned_graph_batched(
                    gt_scene_graphs=gt_scene_graphs,
                    h=x_embed_sum,
                    e=edge_attr_embed_sum,
                    q=q,
        ...
        return x_encoded, edge_attr_encoded, None, gt_scene_graphs

"""
The whole Pipeline. put everything here
"""
class PipelineModel(torch.nn.Module):
    def __init__(self, graph_method="static", graph_top_k=5, graph_hidden_dim=256):
    ...
    def forward(self,
        ...
        x_encoded, edge_attr_encoded, _, sg = self.scene_graph_encoder(gt_scene_graphs, q=q_summary)
        ...
        x_executed = self.gat_seq(x=x_encoded, edge_index=sg.edge_index, edge_attr=edge_attr_encoded, instr_vectors=instr_vectors, batch=sg.batch)
